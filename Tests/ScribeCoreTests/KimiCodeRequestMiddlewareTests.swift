...
struct KimiCodeRequestMiddlewareTests {
    ...
    func injectsHeadersForKimiCom() async throws {
        ...
        let _ = try await middleware.intercept(request, body: nil, baseURL: baseURL, operationID: "chat") { req, body, url in
            capturedRequest = req
    ...
    func handlesEmptyHeaders() async throws {
        ...
        var capturedRequest: HTTPRequest?
        let _ = try await middleware.intercept(request, body: nil, baseURL: baseURL, operationID: "test") { req, body, url in
            capturedRequest = req
            return (HTTPResponse(status: .ok), nil)
        }
    ...
    func passesThroughResponse() async throws {
        ...
        let (response, _) = try await middleware.intercept(request, body: nil, baseURL: baseURL, operationID: "test") { _, _, _ in
    ...
    func preservesMethodAndPath() async throws {
        ...
        var capturedRequest: HTTPRequest?
        let _ = try await middleware.intercept(request, body: nil, baseURL: baseURL, operationID: "stream") { req, body, url in
            capturedRequest = req
            return (HTTPResponse(status: .ok), nil)
        }
