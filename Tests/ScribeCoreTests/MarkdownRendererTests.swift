...
private func plain(_ line: MarkdownLine) -> String {
    line.spans.map(\.plainText).joined()
}

/// Plain text of every line.
private func plainLines(_ lines: [MarkdownLine]) -> [String] {
    lines.map { plain($0) }
}
...
struct MarkdownRendererTests {
    ...
    @Test func whitespaceOnly() {
        let lines = render("   \n  \n  ")
        #expect(lines.allSatisfy { $0.spans.allSatisfy { $0.plainText.trimmingCharacters(in: .whitespaces).isEmpty } })
    }
    ...
    @Test func debugStreamingConsistency() {
        ...
        if oneShot != final {
            ...
            for i in 0..<maxLines {
                let a = i < oneShot.count ? oneShot[i].spans.map(\.plainText).joined() : "<missing>"
                let b = i < final.count ? final[i].spans.map(\.plainText).joined() : "<missing>"
                if a != b {
                    print("Line \(i):")
                    print("  one-shot: '\(a)'")
                    print("  final:    '\(b)'")
    ...
    @Test func debugSinkColors() {
        ...
        func firstSpanKind(of line: TLine) -> String {
        ...
        let expected: [(kind: String, text: String)] = [
            ("plain", "you:"),
            ("plain", "  test"),
            ("empty", ""),
            ("plain", "scribe:"),
            ("plain", "  · answer"),
            ("plain", "### 4. O(n²) re-parsing"),
            ("plain", "1. hello world"),
            ("empty", ""),
        ]

        #expect(completed.count == expected.count)
        for (i, (ek, et)) in expected.enumerated() {
            let text = completed[i].spans.map(\.text).joined()
            #expect(
                firstSpanKind(of: completed[i]) == ek, "line \(i) kind: expected \(ek), got \(firstSpanKind(of: completed[i]))")
    ...
    @Test func inlineCode() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .code(let t) = $0, t.contains("let x = 1") { return true } else { return false } })
    }
    ...
    @Test func inlineCodeWithMarkdownInside() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .code(let t) = $0, t.contains("**not bold**") { return true } else { return false } })
    }
    ...
    @Test func blockQuote() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .blockquote = $0 { return true } else { return false } })
    }
    ...
    @Test func listWithInlineFormatting() {
        ...
        #expect(allSpans.contains { if case .bold = $0 { return true } else { return false } })
        #expect(allSpans.contains { if case .italic = $0 { return true } else { return false } })
        #expect(allSpans.contains { if case .code = $0 { return true } else { return false } })
    }
    ...
    @Test func tableBasic() {
        let md = "| Name | Age |\n| --- | --- |\n| Alice | 30 |\n| Bob | 25 |\n"
        let lines = render(md)
        let allText = lines.flatMap(\.spans).map(\.plainText).joined()
    ...
    @Test func tableHeaderIsBold() {
        ...
        let lines = render(md)
        let allSpans = lines.flatMap(\.spans)
        let headerSpans = allSpans.filter { $0.plainText.contains("Name") }
        #expect(headerSpans.allSatisfy { if case .bold = $0 { return true } else { return false } })
    }
    ...
    @Test func tableWithInlineFormattingInCells() {
        ...
        let lines = render(md)
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .bold(let t) = $0, t.contains("Bold") { return true } else { return false } })
        #expect(allSpans.contains { if case .code(let t) = $0, t.contains("code") { return true } else { return false } })
        #expect(allSpans.contains { if case .italic(let t) = $0, t.contains("italic") { return true } else { return false } })
    }
    ...
    @Test func link() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .link(let t, _) = $0, t.contains("Click here") { return true } else { return false } })
    }

    @Test func image() {
        let lines = render("![alt text](image.png)")
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { $0.plainText.contains("alt text") })
    }
    ...
    @Test func inlineHTML() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { $0.plainText.contains("<b>") })
    }

    @Test func htmlBlock() {
        let lines = render("<div>\n  hello\n</div>")
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { $0.plainText.contains("<div>") })
    }
    ...
    @Test func multipleOverlappingPatterns() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.filter { if case .bold = $0 { return true } else { return false } }.count >= 2)
        #expect(allSpans.filter { if case .italic = $0 { return true } else { return false } }.count >= 2)
        #expect(allSpans.filter { if case .code = $0 { return true } else { return false } }.count >= 2)
    }
    ...
    @Test func literalAsterisksInCodeSpan() {
        ...
        let allSpans = lines.flatMap(\.spans)
        let codeSpans = allSpans.filter { if case .code = $0 { return true } else { return false } }
        #expect(codeSpans.contains { $0.plainText.contains("**not bold**") })
        #expect(allSpans.contains { if case .bold(let t) = $0, t.contains("bold") { return true } else { return false } })
    }
    ...
    @Test func emojiInBold() {
        ...
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .bold(let t) = $0, t.contains("🔥") { return true } else { return false } })
    }

    @Test func emojiInLink() {
        let lines = render("[🚀 launch](https://example.com)")
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { $0.plainText.contains("🚀") })
    }

    @Test func wideUnicode() {
        let lines = render("**日本語** and *中文* and `한국어`")
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.contains { if case .bold(let t) = $0, t.contains("日本語") { return true } else { return false } })
        #expect(allSpans.contains { if case .italic(let t) = $0, t.contains("中文") { return true } else { return false } })
        #expect(allSpans.contains { if case .code(let t) = $0, t.contains("한국어") { return true } else { return false } })
    }
    ...
    @Test func veryLongLineWithPatterns() {
        ...
        let lines = render(s)
        let allSpans = lines.flatMap(\.spans)
        #expect(allSpans.filter { if case .bold = $0 { return true } else { return false } }.count >= 500)
    }
    ...
    @Test func manyConsecutiveBlankLines() {
        ...
        let lines = render(md)
        #expect(
            lines.allSatisfy { $0.spans.allSatisfy { $0.plainText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty } })
    }
    ...
    // MARK: - Line equality / Span merging
    ...
    @Test func safetyNetDoesNotDoubleStyle() {
        ...
        let allSpans = lines.flatMap(\.spans)
        let boldSpans = allSpans.filter { if case .bold = $0 { return true } else { return false } }
        let boldTextTotal = boldSpans.map(\.plainText).joined()
