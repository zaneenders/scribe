  ...
  @Test func createsFileWithContent() async throws {
    ...
    try await withTemporaryDirectory { dir in
      ...
      let json = try! await registry.run(
        name: "write_file", arguments: args, workingDirectory: ScribeFilePath("/tmp"), abortNotifier: AbortNotifier())
      ...
      let reread = try decodeRead(
        try! await registry.run(
          ...
          workingDirectory: ScribeFilePath("/tmp"), abortNotifier: AbortNotifier()))
