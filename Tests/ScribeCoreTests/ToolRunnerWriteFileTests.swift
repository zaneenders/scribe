  ...
  @Test func createsFileWithContent() async throws {
    ...
    try await withTemporaryDirectory { dir in
      ...
      let json = try! await registry.run(
        name: "write_file", arguments: args, workingDirectory: FilePath("/tmp"), log: toolRunnerTestLogger, abortObserver: AbortNotifier()).text
      ...
      let reread = try decodeRead(
        try! await registry.run(
          ...
          workingDirectory: FilePath("/tmp"), log: toolRunnerTestLogger, abortObserver: AbortNotifier()
        ).text
      )
  ...
  @Test func missingContentFieldFails() async throws {
    ...
    try await withTemporaryDirectory { dir in
      ...
      let json = try! await registry.run(
        ...
        workingDirectory: FilePath("/tmp"), log: toolRunnerTestLogger, abortObserver: AbortNotifier()).text
      let fail = try decodeFail(json)
      #expect(fail.ok == false)
