...
Persisted messages have opaque, session-local `MessageCursor` values. Cursors are stable for the lifetime of the session and impose document order; clients must not derive them from array indexes or protocol event sequences. A non-`nil` `currentTurnID` identifies either the active turn or the most recent failed or interrupted uncommitted turn retained by the live agent. Its accumulated content is fetched with the paginated `turnSnapshot(turnID, afterPart, limit)` request. This lets a reconnecting client reconstruct output without relying on transient text events or putting an unbounded turn into `AgentSnapshot`. When a turn is committed, normal message-history pagination is authoritative and `currentTurnID` becomes `nil`. When a turn fails or is interrupted without a commit, its complete partial content and `currentTurnID` remain available until the next turn begins or the agent is unloaded. Starting the next turn atomically replaces that retained partial turn and emits the new turn's state before any of its content events. Thus an attachment snapshot always provides either a durable history boundary or the ID needed to recover the latest uncommitted output; older uncommitted turns are intentionally not retained.
...
- The daemon keeps a `SessionID -> AgentID` map and an `AgentID -> (ConnectionID, ObjectID, ownershipEpoch)` attachment map.
- `openAgent` checks and updates both maps as one daemon-isolated operation. If another connection owns the agent, it returns `sessionInUse`; it never creates a second `SessionHarness`. If that agent is already attached through another handle on the requesting connection, it returns `alreadyAttached` rather than binding a second `ObjectID`.
...
enum TerminalProcessState: Codable, Sendable {
  case running
  case exited(reason: ProcessExitReason)
}

enum ProcessExitReason: Codable, Sendable {
  case code(Int32)
  case signal(number: Int32, coreDumped: Bool)
}
...
struct QueueSnapshotPage: Codable, Sendable {
  ...
  var continuationAfterNumber: UInt64?
}
...
struct TurnSnapshotPage: Codable, Sendable {
  ...
  var continuationAfterPart: UInt64?
}
```
...
Agent mutation preconditions are part of the versioned API. `submit` while running or interrupting appends to the queue; while idle or failed it starts a turn. `clearQueue` is valid in every state and does not affect the active turn. `interrupt` is valid only while running or interrupting and is idempotent while interrupting. `sendQueued` has the interrupt-then-send behavior above; when already interrupting it waits for that shutdown rather than initiating another. `setProfile`, `fork`, and `tldr` require an idle or failed agent and otherwise fail with `agentBusy`; they never change configuration or session identity underneath a running or interrupting turn. A successful `setProfile` affects the next turn. A failed agent may accept `submit`, `setProfile`, `fork`, or `tldr`; beginning successful work leaves the failed state. `close` is valid in every state and performs the documented bounded shutdown. Invalid state combinations fail before mutation with a stable request error.

Agent events:

```text
stateChanged(state: AgentState, currentTurnID: UInt64?)
sessionChanged(previousSessionID: SessionID, session: Session)
userMessage(turnID: UInt64, message: Chunkable<ScribeMessage>)
assistantSectionStarted(turnID: UInt64, sectionID: UInt64, kind: AssistantSectionKind)
assistantText(turnID: UInt64, sectionID: UInt64, text: Chunkable<String>)
assistantFinished(turnID: UInt64, sectionID: UInt64)
toolStarted(turnID: UInt64, toolCallID: String, name: String, arguments: Chunkable<ToolArguments>)
toolFinished(turnID: UInt64, toolCallID: String, result: Chunkable<ToolResult>)
queueChanged(revision: UInt64, queuedMessageCount: UInt32)
profileChanged(name: String)
usageChanged(turnID: UInt64, usage: UsageSnapshot)
turnFinished(turnID: UInt64, outcome: TurnOutcome, latestMessageCursor: MessageCursor?)
```

`Chunkable<T>` denotes the schema-defined inline-or-chunked field representation described below, not a synthesized generic wire type. `AssistantSectionKind`, `ToolArguments`, `ToolResult`, `UsageSnapshot`, and `TurnOutcome` are versioned adjacent-tag wire schemas with golden fixtures. `sectionID` and `toolCallID` correlate events only within the named turn. Text events append to the named section; started and finished events establish its boundaries. `turnFinished.latestMessageCursor` is non-`nil` after a persisted commit and is the history reconciliation boundary. A failed or interrupted turn may have no newly committed cursor; its complete partial content remains available through `turnSnapshot` under the retained `currentTurnID` until the next turn begins or the agent is unloaded. `sessionChanged` is emitted by `fork` and `tldr` after child persistence and updates the attached agent's `sessionID`; its `Session` value also matches the root `sessionCreated` value.

The wire events are stable client-facing messages. The existing internal `AgentEvent` enum is not serialized directly. Each agent event carries a monotonically increasing, agent-local `eventSequence`; applying an event has exactly the field replacement or append behavior described above rather than depending on an internal GUI event type.

`messages` returns a `MessagePage` containing persisted messages in document order and their opaque cursors. Omitting both request cursors returns the newest page; supplying both is invalid. `after` and `before` are exclusive boundaries: the message named by the supplied cursor is not repeated. `hasOlder` and `hasNewer`, not the number of returned items, indicate whether another request in either direction can produce data, and the corresponding cursor is non-`nil` whenever that flag is true. A cursor that is well formed but does not belong to the agent's current session or no longer names a message fails with `invalidCursor`; the client discards that pagination path and starts from an unbounded newest-page request.

`queueSnapshot` freezes one queue revision; omitting `revision` starts at the current revision, while subsequent pages repeat that revision and use `afterNumber` as an exclusive boundary. A non-`nil` `continuationAfterNumber` is exactly the number of the last item returned and is passed unchanged as the next request's exclusive `afterNumber`; it is `nil` only when the queue snapshot is complete. If the queue mutates before paging completes, the continuation fails with `snapshotInvalidated` and the client restarts from the latest revision. Supplying `afterNumber` without `revision` is invalid, and numbers are scoped to their queue revision rather than being durable queue-entry IDs.

`turnSnapshot` returns a `TurnSnapshotPage` with ordered, numbered parts of the active or retained uncommitted turn. `afterPart` is exclusive. A non-`nil` `continuationAfterPart` is exactly the number of the last part returned and is passed unchanged as the next request's exclusive `afterPart`; it is `nil` only when the snapshot is complete. Continuations are scoped to the captured session, queue, or turn and fail with `snapshotInvalidated` if that source is replaced while paging. Responses must fit the negotiated frame size; the daemon may return fewer than `limit` items and a single message, queued message, turn part, or tool payload that cannot fit is returned through chunked message-content frames. Page envelopes themselves are bounded independently of chunked fields. A newly attached GUI loads history and queue contents through these APIs rather than reading daemon-host files.
...
exited(reason: ProcessExitReason)
```
...
A client initializes its next expected byte offset from the first replay frame, or from `TerminalAttachmentSnapshot.nextOutputOffset` (the captured `N`) when replay is empty; an empty replay sends no zero-length replay frame. It then tracks the offset across replay and live output:
...
Each connection has a bounded outbound data queue plus a reserved, bounded control lane. A slow client must not stall the PTY or other clients. When a client falls behind, the daemon drops that terminal handle's queued output and sends `replayInvalidated` through the control lane. No later output is sent on that invalidated handle. The handle enters an invalidated state: `input`, `resize`, `terminateProcess`, and `close` fail with `attachmentInvalidated`, even if it was the controller, while `snapshot` and `detach` remain available. Invalidating a controller immediately transfers control to the oldest non-invalidated observer and emits `accessChanged`; if none exists, the terminal has no controller until a successful recovery attachment becomes controller. This prevents blind input and prevents an invalidated handle from blocking control indefinitely. If the control notice cannot be queued, the daemon closes the entire connection; silently removing a multiplexed handle would not be observable to an idle client.

Recovery uses `attachTerminal` with a fresh, unused `ObjectID`. The operation atomically replaces any invalidated handle for that terminal on the same connection and establishes a new snapshot/replay/live-output baseline. The old handle is closed after the new baseline is installed. If recovery fails, the old handle remains invalidated and may only be inspected or detached; retries use another fresh ID. If the retained range no longer includes the missing bytes, the GUI performs the best-effort reset and replay described above.
...
A saved remote daemon contains a display name and OpenSSH host alias. A focused Swift parser reads `~/.ssh/config`; NIOSSH uses the resolved host, user, port, keys, jump host, and known-host policy. Version 1 supports `Host`, negated host patterns, `Hostname`, `User`, `Port`, `IdentityFile`, `IdentitiesOnly`, `ProxyJump`, `UserKnownHostsFile`, `StrictHostKeyChecking`, and recursive `Include` with cycle and file-count limits. It rejects a selected host that depends on unsupported `Match`, `ProxyCommand`, certificate, PKCS#11/security-key-provider, or canonicalization behavior instead of silently connecting with different semantics. SSH-agent identities and encrypted key prompting are explicit client capabilities, not config-parser side effects. Golden tests compare supported resolution against OpenSSH and verify rejection of unsupported selected-host directives.
...
Chunks for one transfer are emitted in increasing contiguous `byteOffset` order starting at zero. The final flag is set exactly when `byteOffset + bytes.count == byteLength`; a zero-length value stays in JSON and is never chunked. Different transfers and request IDs may interleave, so receivers reassemble by `transferID`. For request and response transfers, the chunk frame repeats the descriptor frame's nonzero `requestID` and `objectID`; mismatches are protocol errors. Event chunks use request ID zero and repeat the event's `objectID`, relying on the connection-unique transfer ID for disambiguation. A transfer is valid only after its descriptor has been received, its encoding and semantic field come from that descriptor, and duplicate, overlapping, out-of-range, mismatched-length, or unknown-transfer chunks are protocol errors. A request is not accepted for execution and produces no effects until its JSON descriptor and every referenced transfer have completed and decoded as the declared typed fields. Likewise, a response is complete, and an event is eligible for sequence application, only after its descriptor and all referenced transfers complete; later events for the same subscription are not applied ahead of it. Connection or subscription closure abandons incomplete transfers.

This mechanism applies in either direction to every schema field marked chunkable, including submitted message text, message text, reasoning, image content, tool arguments and results, and unfinished-turn parts. Stable numeric `code` values distinguish `contentChunk` from terminal `replay`, `output`, and `input`, so their raw layouts are never inferred from context.

For a state-changing request with chunked fields, the descriptor must contain its `OperationID`. On receiving a valid descriptor, the daemon reserves bounded transfer capacity and one operation-record slot, then records a provisional `awaitingContent` entry for that ID. It does not begin mutation effects. `operationResult` reports that entry as `inProgress`; concurrent mutation requests reusing the ID fail retryably with `operationAwaitingContent` because canonical argument equality cannot be established before assembly. After assembly, canonical typed arguments are stored and normal deduplication rules apply. Transfer timeout, malformed content, or disconnect completes the provisional entry with a retained `incompleteRequest` failure and no mutation effects. This makes `operationResult` definitive after uncertain delivery and prevents a partially delivered `submit` or other mutation from executing. Read-only chunked requests use the same assembly rules without an operation record.
...
Every state-changing request carries an `OperationID`, generated once and reused only when retrying that same logical mutation. Read-only requests, attachment, and detach do not need one. The daemon records an operation's in-progress state before beginning its effects and records its terminal success or error before answering its callers. A duplicate with the same opcode, target live identity, and canonical arguments joins the in-progress operation or returns the cached result without executing again; reuse with different inputs returns `operationIDConflict`. Canonical arguments are the typed, schema-normalized semantic values, not source JSON bytes. For large arguments such as terminal input, a record may retain a collision-resistant digest plus the canonical argument length instead of retaining the bytes, but it must still detect conflicting reuse.

Connection-local handle fields are excluded from mutation identity. In particular, `createTerminal.newObjectID` is validated and used only for the first executing request on its originating connection; its canonical mutation arguments are working directory, columns, and rows. The retained success is `createdTerminal(terminalID, terminalSnapshot)`, not a connection-local attachment. An identical duplicate on the original connection returns that result and binds no second handle; a joiner or retry on another connection receives the same live identity without binding its requested `newObjectID` and must call `attachTerminal` with a fresh handle after reacquiring the terminal through a root snapshot. The original first-execution response additionally returns its atomically established `TerminalAttachmentSnapshot`. If that response is lost, `operationResult` never recreates the handle. No other version-1 mutation establishes a connection-local handle.
...
enum OperationTerminalResult: Codable, Sendable {
  case success(OperationSuccess)  // opcode-specific semantic result; never contains ObjectID
  case failure(OperationFailure)  // stable error code and versioned details
}
```

`operationResult` returns this union without recreating a connection-local handle. `completed` contains the same typed semantic success or error as the original mutation; it excludes connection-local attachment metadata that may have accompanied the first-execution response. For `inProgress`, the client may poll after the optional delay or resend the identical request with the same `OperationID` to join it; it must not create a new ID. `unknown` means the daemon cannot safely determine whether the mutation occurred.
...
5. **Local daemon transport.** Add `scribe-daemon serve`, per-data-directory singleton locking, secure Unix socket lifecycle and peer authorization, and map one connection onto the already-tested terminal runtime. Add the separately versioned and signed process-reaper helper, enforce daemon/helper protocol compatibility before launch, and add concurrent startup, stale-artifact recovery, create, attach, input, resize, bounded process termination, daemon-crash cleanup, and object close integration tests. Chat support is not required here.
