struct MoveUpVisitor: RawVisitor {
  ...
  mutating func process(previous block: some Block) {
    visit(block)
  }
  ...
  mutating func visitArray<B: Block>(_ array: _ArrayBlock<B>) {
    ...
    for (index, child) in array.children.enumerated() {
      ...
      case .breakOutOfChild:
        ...
        if let prevIndex {
          currentHash = hash(contents: "\(ourHash)\(#function)\(prevIndex)")
          process(previous: array.children[prevIndex])
        }
        return
      case .findingSelected, .foundSelected, .selectionUpdated:
        ()
